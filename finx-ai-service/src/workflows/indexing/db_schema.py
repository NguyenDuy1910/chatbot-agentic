...
if __name__ == "__main__":
    """Example usage of the DB Schema Indexing workflow."""
    import asyncio
    import os
    
    async def main():
        """
        Example of indexing MDL schema.
        
        Requirements:
        - Set GOOGLE_API_KEY environment variable
        - Qdrant running at http://localhost:6333
        - MDL file at ../../../finx-engine/data/mdls.json
        """
        ...
        if not api_key:
            ...
            print("   Continuing with mock embeddings for demo...")
        
        # Path to MDL file
        mdl_file = "../../../finx-engine/data/mdls.json"
        
        print("\n" + "="*60)
        print("DB SCHEMA INDEXING DEMO")
        print("="*60)
        print(f"MDL file: {mdl_file}")
        print(f"Qdrant: http://localhost:6333")
        print(f"Collection: db_schema")
        print(f"Chunking: table-level")
        print("="*60 + "\n")
        ...
        try:
            ...
            print(f"\n❌ Error: MDL file not found")
            print(f"   Please ensure {mdl_file} exists")
        except Exception as e:
            print(f"\n❌ Error: {e}")
            import traceback
            traceback.print_exc()
