...
Usage:
    ...
    python -m src.workflows.intent_recommendation.demo --test text2sql
    python -m src.workflows.intent_recommendation.demo --test general
    python -m src.workflows.intent_recommendation.demo --test all
"""
...
from src.workflows.intent_recommendation import (
    IntentRecommendationGraph,
    create_initial_intent_recommendation_state,
)
from src.workflows.intent_recommendation.llm_helper import create_llm_generator
...
async def run_test_case(
    ...
    # Add LLM generator to context if available
    try:
        generator = create_llm_generator(provider="gemini", temperature=0.7)
        initial_state["context"] = {
            "generator": generator,
        }
        logger.info("✅ LLM generator added to context")
    except Exception as e:
        logger.warning(f"⚠️  Could not create LLM generator: {e}")
        logger.info("Running in demo mode without real LLM")
        initial_state["context"] = {}
